    ...
    policies: {
        loggedIn: function(req, res, next) {
            if (req.session.user) {
                ...
                res.redirect(this.settings.login_url + '?' + querystring.stringify({
                    return_url: q
                }));
            }
        },
                    ...
                    req.model.consent.findOne({
                        ...
                        reqsco.forEach(function(scope) {
                            ...
                            if (!consent) {
                                req.session.scopes[scope] = {
                                    ismember: false,
                                    explain: self.settings.scopes[scope]
                                };
                        ...
                        Q.allSettled(promises).then(function(results) {
                            ...
                            if (redirect) {
                                ...
                                deferred.reject({
                                    type: 'redirect',
                                    uri: self.settings.consent_url + '?' + querystring.stringify({
                                        return_url: q
                                    })
                                });
                            } else {
...
OpenIDConnect.prototype.userInfo = function() {
    ...
    return [
        ...
        function(req, res, next) {
            req.model.access.findOne({
                    token: req.parsedParams.access_token
                })
                .exec(function(err, access) {
                    if (!err && access) {
